...
- ONLY runs if:
    STRANDS_ENABLED=true  AND  STRANDS_AVAILABLE=True
- Fails CLOSED:
    - On import errors
    - On timeouts
...
This file can exist safely in Phase 5:
- With STRANDS_ENABLED defaulting to "false"
- Without strands SDK installed
"""
...
class SafeStrandsAgent:
    ...
    def __init__(self, name: str, instructions: str, allowed_topics: List[str]):
        ...
        if self.enabled and Agent is not None:
            try:
                ...
                logger.warning(
                    "Failed to initialize SafeStrandsAgent '%s': %s. "
                    "Disabling Strands for safety.",
                    name,
                    e,
                )
                self.enabled = False
    ...
    def generate(self, user_text: str, base_response: str) -> str:
        ...
        def _run():
            ...
            return self._agent.run(prompt)  # type: ignore[attr-defined]
